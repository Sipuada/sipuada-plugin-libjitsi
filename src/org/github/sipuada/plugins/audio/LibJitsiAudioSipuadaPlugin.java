...
public class LibJitsiAudioSipuadaPlugin implements SipuadaPlugin {

	private final Logger logger = LoggerFactory.getLogger
		(LibJitsiAudioSipuadaPlugin.class);
	...
	public LibJitsiAudioSipuadaPlugin(String identifier) {
		...
		try {
			if (sdp.getConnection() != null) {
				...
				logger.debug("%% {} could not find parent data connection "
					+ "address: {}! %%", LibJitsiAudioSipuadaPlugin
	...
	public boolean performSessionSetup(String callId, SipUserAgent userAgent) {
		...
		for (SupportedAudioCodec supportedAudioCodec : streams.keySet()) {
			...
			MediaFormat mediaFormat = mediaService.getFormatFactory()
				...
				supportedAudioCodec.getClockRate());
